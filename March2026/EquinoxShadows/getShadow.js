...
const getShadow = (time) => {
    let splitTime = time.split(':');
    let hours = Number(splitTime[0]);
    let minutes = Number(splitTime[1]) === 30 ? .5 : 0;
    time = hours + minutes;
    let timeToNoon = Math.abs(time - 12);
    let shadow = timeToNoon ** 3;
    if ((hours === 12 && minutes === 0) || hours >= 18 || hours < 6) {
        return 'No shadow';
    }
    else if(hours >= 12) {
        return `${shadow}ft east`;
    }
    else {
