...
function detectRoast(beans) {
    let apostropheCount = beans.match(/'/gm) || [];
    let dashCount = beans.match(/-/gm) || [];
    let periodCount = beans.match(/\./gm) || [];
    let beansCount = beans.length;
    let roastCount = (apostropheCount.length * 1) + (dashCount.length * 2) + (periodCount.length * 3);
    let roastAverage = roastCount / beansCount;
    if(roastAverage < 1.75) {
        return "Light";
    }
    else if(roastAverage >= 1.75 && roastAverage <= 2.5) {
        return "Medium";
    }
    else if(roastAverage > 2.5){
        return "Dark";
    }
}

module.exports = detectRoast;