...
function getFastestSpeed(times) {
    ...
    for (let i = 0; i < times.length; i++) {
        segmentSpeeds.push(Math.round((segmentLengths[i] / times[i]) * 100) / 100);
    }
    const fastestSpeed = Math.max(...segmentSpeeds);
    return `The luger's fastest speed was ${fastestSpeed} m/s on segment ${segmentSpeeds.indexOf(fastestSpeed) + 1}.`;
}

module.exports = getFastestSpeed;