...
const findPawnMoves = (position) => {
    ...
    switch (position) {
        ...
        case"G2": 
        case "H2":
            possibleMoves.push(`${position[0]}3`, `${position[0]}4`);
            break;
        default:
            let row = Number(position[1]);
            let newRow = row < 8 ? row + 1 : row;
            possibleMoves.push(`${position[0]}${newRow}`);
            break;
    }
    return possibleMoves;
}

module.exports = findPawnMoves;