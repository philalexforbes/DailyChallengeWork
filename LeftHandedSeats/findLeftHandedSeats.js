...
function findLeftHandedSeats(table) {
    let openLeftHandedSeats = 0;
    table[1].reverse(); //Reversing because the second row is facing towards the first this allows for simplified logic in the below loop and if statements
    for (let i = 0; i < table.length; i++){
        for(let j = 0; j < table[i].length; j++){
            if((i+1) !== table[i].length){
                if(table[i][j] === 'U' && table[i][j+1] !== 'R'){
                    openLeftHandedSeats += 1;
                }
            }  
        }
    }
    return openLeftHandedSeats;
}

module.exports = findLeftHandedSeats;